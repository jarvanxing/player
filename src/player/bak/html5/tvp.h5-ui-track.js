...
(function(x, $) {
	...
	$.extend(x.Html5UI.fn, {
		buildtrack: function(player, $video, $control, $UILayer) {
			...
			$.each(x.html5skin.elements.track, function(k, v) {
				$elements[k] = $control.find(v);
			});

			$element.caption = $('<div class="x_track"><span></span></div>').appendTo($UILayer),
			...
			function renderSrtListPannel(defValue) {
				...
				var defName = x.html5lang.getSrtName(defValue),
					listKV = {},
					html = "";
